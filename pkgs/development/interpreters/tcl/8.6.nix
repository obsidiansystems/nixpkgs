...
callPackage ./generic.nix (
  ...
  // rec {
    ...
    patches =
      ...
      # Cygwin wants the DLL in `bin` and an import library, `libtcl8.6.dll.a`,
      # in `lib`; that is what `tclConfig.sh` describes when it says
      # `-L${libdir} -ltcl8.6`. This branch's `unix` build system builds no
      # import library at all, so nothing can link against Tcl. Upstream fixed
      # that on 9.0 but never backported it.
      #
