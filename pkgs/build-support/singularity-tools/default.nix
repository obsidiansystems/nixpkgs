  ...
  mkLayer = {
   ...
   buildImage = {
     ...
     extraSpace
         ...
         configuration = {
           imports = [
             "${path}/nixos/modules/virtualisation/singularity-container.nix"
             "${path}/nixos/modules/profiles/minimal.nix"
             "${path}/nixos/modules/config/shells-environment.nix"
           ];
           environment.systemPackages = contents ++ [ bash stdenv ];
           singularityContainer = {
             inherit diskSize extraSpace runAsRoot runScript;
           };
           ...
           fonts = {
             ...
             enableFontDir = true;
             fonts = [ ubuntu_font_family ];
           };
         };
     }).config.system.build.singularityContainer;
}
#  buildImage = {
#    name,
#    contents ? [],
...
#            mkdir -p bin run/user etc nix/store
#            for c in $(cat $layerClosure) ; do
#              cp -ar $c ./$c
#              for f in "$c/etc/*" ; do
#                echo $f
#                if [ ! -e etc/$(basename $f) ] ; then
#                  ln -s $f etc/
#                fi
#              done
#            done
