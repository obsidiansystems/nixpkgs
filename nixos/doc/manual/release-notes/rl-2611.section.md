...
- Boost is now built as one package per Boost library, each from its own
  `boostorg` repository with CMake, rather than as a single `b2` build of the
  release tarball. The libraries are exposed as `boostPackages`, and per
  release as `boostPackages_1_89` and so on, so a dependant can depend on just
  what it uses: `boostPackages.context` has nothing in its closure but libc and
  libstdc++, where `boost` pulls in all of Boost and `icu4c`.

  `boost` itself is unchanged as far as dependants are concerned. It is now
  `boostPackages.everything`, which symlinks the whole set into one tree, so
  `${boost}/lib`, `${boost.dev}/include`, `find_package(Boost)` and
  `boost.override { enablePython = true; }` all still work, and `boost189` and
  the other versioned attributes keep their names.

  Upstream's CMake builds fewer compiled libraries than `b2` did: Boost.Regex
  and the `math_*` libraries are header-only there. Nixpkgs still builds
  `libboost_regex.so`, because autoconf's `AX_BOOST_REGEX` link-tests
  `-lboost_regex` and cannot be satisfied by headers, but nothing links the
  `math_*` libraries and they are gone. `libboost_system.so` was already absent
  under `b2` and remains so.
